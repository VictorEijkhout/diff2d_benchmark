...
\item
  ...
  which is allocated with size $(m+b)\times(n+b)$.
...
\begin{figure}[t]
  ...
  \hbox\bgroup %%\begin{multicols}{2}
    ...
    \end{tikzpicture}
    %% \columnbreak
    \tikzsetnextfilename{diff2d-procs-sp}
    \begin{tikzpicture}
      \begin{loglogaxis}[
          title=Speedup,
        ...
        \addplot table[y=skx]{plots/gcc-oned-skx-26000-sp.csv};
        \addplot table[y=clx]{plots/gcc-oned-clx-26000-sp.csv};
        \addplot table[y=icx]{plots/gcc-oned-icx-26000-sp.csv};
        \addplot table[y=spr]{plots/gcc-oned-spr-26000-sp.csv};
        \addplot table[y=grx]{plots/gcc-oned-grx-26000-sp.csv};
      \end{loglogaxis}
    \end{tikzpicture}
...
We need to start by explaining how we measure the bandwidth.
