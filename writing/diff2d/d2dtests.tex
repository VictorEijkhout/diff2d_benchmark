...
takes care (in a sense that we will later define)
...
\item Computing $\gamma$ is a all-reduction:
  each processing element $p$ first computes $\gamma_p=\surd\sum_{i\in I_p}y_i^2$
  independently,
  and $\gamma=\surd\sum_p\gamma_p$ is then formed by combination, and
